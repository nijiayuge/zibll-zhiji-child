...
function zib_this_charts_data($order_type = 0)
{
    ...
    $order_type_where .= " and pay_type not in ('points','balance')";
...
function zib_this_get_time_filling($cycle, $time)
{
    ...
    if (strtotime($time_end) < strtotime($time_start)) {
        throw new Exception(__('结束时间不能小于开始时间', 'zib_language'));
    }
...
<div class="pay-container">
    ...
    <div class="row-6">
        <div class="box-panel highcharts">
            <div class="highcharts-title"><?php echo esc_html__('有效收款单量', 'zib_language'); ?></div>
            <div style="margin:0 -30px -20px 0px;">
                <div id="highcharts_count" style="height:300px"></div>
            </div>
        </div>
    </div>
    <div class="row-6">
        <div class="box-panel highcharts">
            <div class="highcharts-title"><?php echo esc_html__('有效收款金额', 'zib_language'); ?></div>
            <div style="margin:0 -30px -20px 0px;">
                <div id="highcharts_price" style="height:300px"></div>
            </div>
        </div>
    </div>
    <div class="px12"><?php echo esc_html__('当前统计不含余额、积分付款的数据', 'zib_language'); ?></div>
    <script type="text/javascript">
        (function($, document) {
            $(document).ready(function($) {
                var option_1 = {
                    legend: {
                        data: ['<?php echo esc_js(__('全部订单', 'zib_language')); ?>', '<?php echo esc_js(__('购买会员', 'zib_language')); ?>']
                    },
                    tooltip: {
                        trigger: 'axis'
                    },
                    ...
                    series: [{
                        name: '<?php echo esc_js(__('全部订单', 'zib_language')); ?>',
                        data: <?php echo json_encode($charts_data['count']); ?>,
                        type: 'line',
                        smooth: true
                    }, {
                        name: '<?php echo esc_js(__('购买会员', 'zib_language')); ?>',
