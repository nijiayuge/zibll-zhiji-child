    ...
    <div v-loading="loading.express_dialog">
        <div v-if="express_dialog_data.express_data.traces">
            <div v-if="express_dialog_data.address_data.name" class="mb20 text-box">
                <div class="flex">
                    <div class="opacity8 mr20 flex0"><?php echo esc_html__('收件人', 'zib_language'); ?></div>
                    <div class="flex1">
                        <div class="flex ac">
                    ...
                    <el-button type="primary" plain size="small" @click="copyAddress(express_dialog_data.address_data)"><?php echo esc_html__('复制地址', 'zib_language'); ?></el-button>
                </div>
            </div>
            <el-timeline>
                <el-timeline-item
...
<el-dialog
    ...
    title="<?php echo esc_attr__('手动补单', 'zib_language'); ?>"
    ...
    <el-form v-if="[-1,0].includes(~~payment_dialog_data.status)" class="order-edit-form" :model="payment_dialog_data"
        ...
        <div class="text-box mb10">
            ...
            <div class="flex ac jsb mt6">
                <div class="opacity5 mr10 flex0"><?php echo esc_html__('下单时间：', 'zib_language'); ?></div>
                <div>{{payment_dialog_data.create_time}}</div>
            </div>
            <div class="flex ac jsb mt6">
                <div class="opacity5 mr10 flex0"><?php echo esc_html__('购买用户：', 'zib_language'); ?></div>
                <div>{{payment_dialog_data.user_info.name || '<?php echo esc_js(__('未知', 'zib_language')); ?>'}}</div>
            </div>
            <div class="flex ac jsb mt6" v-if="payment_dialog_data.order_data.remark">
                <div class="opacity5 mr10 flex0"><?php echo esc_html__('下单备注：', 'zib_language'); ?></div>
                <div>{{payment_dialog_data.order_data.remark || '<?php echo esc_js(__('无', 'zib_language')); ?>'}}</div>
            </div>
        ...
        <el-form-item label="<?php echo esc_attr__('支付方式', 'zib_language'); ?>">
            <el-select v-model="payment_dialog_data.payment_method" placeholder="<?php echo esc_attr__('请选择支付方式', 'zib_language'); ?>">
                <el-option v-for="(name, key) in payment_methods_names" :key="key" :label="name" :value="key"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="<?php echo esc_attr__('支付金额', 'zib_language'); ?>">
            <el-input v-model="payment_dialog_data.pay_price" placeholder="<?php echo esc_attr__('请输入支付金额', 'zib_language'); ?>"></el-input>
        </el-form-item>
        <el-form-item label="<?php echo esc_attr__('支付订单号', 'zib_language'); ?>">
            <el-input v-model="payment_dialog_data.pay_num" placeholder="<?php echo esc_attr__('请输入支付订单号', 'zib_language'); ?>"></el-input>
        </el-form-item>
        <el-form-item>
            <el-button type="primary" :loading="loading.payment_dialog_submit_but" :disabled="!payment_dialog_data.payment_method || !payment_dialog_data.pay_price || !payment_dialog_data.pay_num" @click="paymentSubmit"><?php echo esc_html__('确认提交', 'zib_language'); ?></el-button>
        </el-form-item>

    </el-form>
    <div v-else class="flex jc ac" style="height: 100%;">
        <el-empty description="<?php echo esc_attr__('当前订单无法手动支付', 'zib_language'); ?>"></el-empty>
    </div>
</el-dialog>
